...
/datum/reagent/inverse/penthrite
	...
	var/static/list/trait_buffs = list(
		...
		TRAIT_NOSOFTCRIT,
		TRAIT_STABLEHEART,
		TRAIT_NOPASSOUT, // MONKESTATION EDIT: Replacement for constantly removing TRAIT_KNOCKEDOUT
	)
...
/datum/reagent/inverse/penthrite/on_mob_dead(mob/living/carbon/affected_mob, seconds_per_tick)
	...
	affected_mob.revive() // MONKESTATION EDIT: Modified to use less snowflake shit for revival.
	affected_mob.set_resting(new_resting = FALSE, silent = TRUE, instant = TRUE) //Please get up, no one wants a deaththrows juggernaught that lies on the floor all the time
