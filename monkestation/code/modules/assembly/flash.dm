/obj/item/assembly/flash/examine(mob/user)
	...
	if(!QDELETED(blood_bond))
		...
		if(REF(blood_bond) in flash_handler?.sources)
			...
			. += span_warning("Conversion will fail if the target is either dead, unconscious, SSD, mindshielded, someone else's brother, or if they are targeted by your objectives.") // monkestation edit: remove security check
