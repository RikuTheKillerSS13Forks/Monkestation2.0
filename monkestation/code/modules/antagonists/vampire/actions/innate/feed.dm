...
/datum/action/cooldown/vampire/feed/proc/can_feed(feedback)
	...
	if (!target_antag_datum && HAS_TRAIT(target, TRAIT_NOBLOOD))
		if (feedback)
			user.balloon_alert(user, "they lack blood!")
		return FALSE
	if (target_antag_datum && target_antag_datum.current_lifeforce <= 0)
		if (feedback)
			user.balloon_alert(user, "they lack lifeforce!")
		return FALSE
	return TRUE
...
/datum/action/cooldown/vampire/feed/toggle_on()
	...
	if (is_neck_feed)
		...
		to_chat(target, span_userdanger("[user] sinks their fangs into your neck!"))
	else
		user.visible_message(
			message = span_notice("[user] gently sinks their fangs into [target]'s wrist."),
			self_message = span_notice("You gently sink your fangs into [target]'s wrist."),
			ignored_mobs = list(target),
		)
		to_chat(target, span_userdanger("[user] gently sinks their fangs into your wrist.")),
