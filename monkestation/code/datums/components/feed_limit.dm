...
#define FEED_LIMIT BLOOD_VOLUME_NORMAL
...
/datum/component/feed_limit
	...
	/// The amount of blood drained from us so far. Increased via a signal sent by the feed action, decays over time.
	var/blood_drained = 0
...
/datum/component/feed_limit/proc/RegisterWithMob(mob/living/target)
	...
	RegisterSignal(target, COMSIG_MOB_FED_FROM, PROC_REF(on_fed_from))
...
/datum/component/feed_limit/proc/UnregisterFromMob(mob/living/target)
	...
	UnregisterSignal(target, list(COMSIG_ATOM_EXAMINE, COMSIG_MOB_FED_FROM, COMSIG_LIVING_LIFE))
...
/datum/component/feed_limit/proc/on_examine(mob/living/target, mob/examiner, list/examine_text)
	...
	if (limit_reached)
		...
		return
	if (blood_drained > BLOOD_VOLUME_NORMAL * 0.5)
...
/datum/component/feed_limit/proc/on_fed_from(mob/living/target, mob/living/carbon/human/vampire, amount)
	SIGNAL_HANDLER
	if (limit_reached)
		return

	blood_drained = min(blood_drained + amount, FEED_LIMIT)
	if (blood_drained < FEED_LIMIT)
		return
...
/datum/component/feed_limit/proc/on_life(mob/living/target, seconds_per_tick, times_fired)
	SIGNAL_HANDLER
	blood_drained -= DECAY_RATE * DELTA_WORLD_TIME(SSmobs)
	if (blood_drained <= 0)
		qdel(src)

/datum/component/feed_limit/proc/get_remaining_lifeforce()
	return limit_reached ? 0 : (FEED_LIMIT - blood_drained) * BLOOD_TO_LIFEFORCE
